...
const getters = {
  ...
  // msgErrorProduit: state => state.msgErrorProduit,
    ...
    listeNewAddProduits: state => state.listeNewAddProduits,
    listeNewAddProdError: state => state.listeNewAddProdError,
...
const actions = {
  ...
  //           })
  //   },
    GET_NEW_ADD_PRODUIT({ commit }) {
        Api.get("/api/products/newAddProduct/8")
            .then(response => {
                commit("setListNewAddProduits", response.data)
                console.log(response.data);
            })
            .catch((error) => {
                commit("setlisteNewAddProdError", error)
            })
    },
...
const mutations = {
  ...
  setListNewAddProduits: (state, listeNewAddProduits) => (state.listeNewAddProduits = listeNewAddProduits),
  setlisteNewAddProdError: (state, listeNewAddProdError) => (state.listeNewAddProdError = listeNewAddProdError),
