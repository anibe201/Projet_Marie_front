  ...
  POST_PRODUIT({ commit }, data) {
    ...
    GET_ALL_PRODUIT({ commit }) {
        Api.get("/api/products/getAllProducts")
            ...
            .catch((error) => {
                commit("setlisteProdError", error)
            })
    },

    GET_SINGLE_PRODUIT({ commit }, id) {
        Api.get("/api/products/getProduct/" + id)
            .then(response => {
...
const mutations = {
  ...
  setlisteProdError: (state, listeProdError) => (state.listeProdError = listeProdError),
